            ...
            vim.keymap.set('n', '<leader>ip', ':PasteImage<CR>', { desc = 'Paste image' })
                      ...
                      vim.api.nvim_create_autocmd("FileType", {
                        ...
                        callback = function()
                          ...
                          vim.opt_local.showbreak = "  ↳ "  -- Indent wrapped lines
                          vim.opt_local.breakindent = true
                          vim.opt_local.breakindentopt = "shift:2,min:20"

                          -- Smoother scrolling for wrapped lines
                ...
                -- Configure markview with updated syntax and doom one theming
                require("markview").setup({
                  ...
                  code_blocks = {
                    enable = true,
                    style = "simple",
                    border_hl = "MarkviewCodeBlockBorder", -- Different name to avoid conflicts
                  },
                ...
                -- Set all highlight groups after markview setup
                local function set_doom_one_markdown_highlights()
                  -- Your preferred heading color order
                  ...
                  -- Headings
                  vim.api.nvim_set_hl(0, "MarkviewHeading1", { fg = blue, bold = true })
                  vim.api.nvim_set_hl(0, "MarkviewHeading2", { fg = magenta, bold = true })
                  vim.api.nvim_set_hl(0, "MarkviewHeading3", { fg = teal, bold = true })
                  vim.api.nvim_set_hl(0, "MarkviewHeading4", { fg = cyan, bold = true })
                  vim.api.nvim_set_hl(0, "MarkviewHeading5", { fg = violet, bold = true })
                  vim.api.nvim_set_hl(0, "MarkviewHeading6", { fg = dark_cyan, bold = true })

                  -- Code styling - only for borders and inline code, NOT block content
                  vim.api.nvim_set_hl(0, "MarkviewInlineCode", { fg = "#b18c00", bold = true, underline = true })
                  vim.api.nvim_set_hl(0, "MarkviewCodeBlockBorder", { fg = "#5B6268", bg = "#2d333b" }) -- Border only

                  -- Text formatting
                  vim.api.nvim_set_hl(0, "MarkviewBold", { fg = "#c5cdd8", bold = true })
                  vim.api.nvim_set_hl(0, "MarkviewItalic", { fg = "#79c6ff", bold = true })
                  ...
                  -- REMOVE these lines that override treesitter syntax highlighting:
                  -- vim.api.nvim_set_hl(0, "@markup.raw.block", { fg = "#c678dd", bg = "#2d333b" })

                  -- Only set fallback highlights for inline code and basic markup
                  vim.api.nvim_set_hl(0, "@markup.strong", { fg = "#c5cdd8", bold = true })
                  vim.api.nvim_set_hl(0, "@markup.italic", { fg = "#79c6ff", bold = true })
                  vim.api.nvim_set_hl(0, "@markup.raw.markdown_inline", { fg = "#b18c00", bold = true, underline = true })
                ...
                -- Apply immediately and on markdown files
                ...
                -- Ensure treesitter highlighting works in code blocks
                vim.api.nvim_create_autocmd("FileType", {
                  pattern = "markdown",
                  callback = function()
                    -- Set conceallevel for better rendering but preserve syntax highlighting
                    vim.opt_local.conceallevel = 2
                    vim.opt_local.concealcursor = 'nc'
                  end,
                })

    -- Better spacing and typography for markdown
    vim.api.nvim_create_autocmd("FileType", {
      pattern = "markdown",
      callback = function()
        -- Better line spacing
        ...
        -- Improved wrapping and indentation
        vim.opt_local.wrap = true
        vim.opt_local.linebreak = true
        vim.opt_local.breakindent = true
