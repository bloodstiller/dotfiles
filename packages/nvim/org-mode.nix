...
{ lib, pkgs, ... }: {
  vim.luaConfigRC.extraPlugins = ''
    ...
    --  -- Dynamic line wrapping with vim-pencil based on window size
    --  vim.api.nvim_create_autocmd('FileType', {
    --      pattern = 'org',
    --      callback = function()
    --          vim.opt_local.conceallevel = 3
    --          
    --          -- Function to set wrapping mode based on window width
    --          local function set_pencil_mode()
    --              local win_width = vim.api.nvim_win_get_width(0)
    --              
    --              if win_width > 200 then
    --                  -- Wide window: use hard wrapping with column guide
    --                  vim.cmd('HardPencil')
    --                  vim.opt_local.colorcolumn = "100"
    --              else
    --                  -- Narrow window: use soft wrapping, no column guide
    --                  vim.cmd('SoftPencil')
    ...
    --              buffer = 0,
    --              callback = set_pencil_mode
    --          })
    --      end,
    --  })

             --- Dynami Line wrapping based on window size
                 vim.api.nvim_create_autocmd('FileType', {
                     pattern = 'org',
                     callback = function()
                         vim.opt_local.conceallevel = 3
